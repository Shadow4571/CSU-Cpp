...
class binary_tree {
    ...
    // Метод прохода по нодам и проверки является ли дерево BST
    bool isBSTUtil(tree_node* Node, K Min, K Max) {
        if(Node == nullptr)
            return true;
        ...
        return this->isBSTUtil(Node->getLeft(), Min, Node->getKey() - 1) && this->isBSTUtil(Node->getRight(), Node->getKey() + 1, Max);
    }
    ...
    V get(const K &Key) {
        if(!this->contains(Key))
            throw string("Cannot get element by key\n");
    ...
    pair<K, V> getPrevValue(K Key) {
        if(!this->contains(Key))
            throw string("Tree does not contains key\n");
	...
	// Найти первый элемент, который больше, чем ключ
    pair<K, V> findNextEntry(K Key) {
        if(this->contains(Key)) {
            tree_node* temp = this->findNode(Key);
            if(temp->getRight() == nullptr) {
                ...
                if(temp == nullptr)
                    throw string("Cannot find key bigger then input key\n");
        ...
        return temp == nullptr ? throw string("Cannot find key bigger then input key\n") : pair<K, V>(temp->getKey(), temp->getValue());
    }
    
	// Проверить является ли дерево BST
    bool isBST() {
        vector<pair<K, V>> allNodes = this->getAllNodes(ST_minMax);
        return this->isBSTUtil(this->Root, allNodes[0].first, allNodes[allNodes.size() - 1].first);
    }
...
int main() {
    binary_tree<int, string> tree = binary_tree<int, string>(5, "5");
    tree.add(-1, "-1"); tree.add(8, "8");
    tree.add(3, "3"); tree.add(0, "0");
    tree.add(4, "4"); tree.add(2, "2");
    tree.add(6, "6"); tree.add(10, "10");
    tree.add(7, "7");
    
    tree.printTree();
    
    pair<int, string> findSeven = tree.findNextEntry(7);
    pair<int, string> findOne = tree.findNextEntry(1);
    
    cout << "Node bigger then 7: " << findSeven.first << endl;
    cout << "Node bigger then 1: " << findOne.first << endl;
    
    try {
        cout << "Node bigger then 12: " << tree.findNextEntry(12).first;
    } catch(string Exp) {
        cout << Exp;
    }
    
    cout << endl << boolalpha << "IS BST: " << tree.isBST() << endl;
